...
(function() {
  ...
  App.CommonController = Ember.ResourceController.extend({
    ...
    _prepareResourceRequest: function(params) {
      ...
      if (this.get('filter.map_id')) {
        params.url += '&map_id=' + this.get('filter.map_id');
      }
    },
    ...
    loadAll: function(json) {
      ...
      var pageStart = pagination.pagesStart || pagination['pages.start'];
      var pagesStop = pagination.pagesStop || pagination['pages.stop'];
      var pagesCurrent = pagination.pagesCurrent || pagination['pages.current'];

      for (i = pageStart; i <= pagesStop; i++) {
        page = {
          number: i,
