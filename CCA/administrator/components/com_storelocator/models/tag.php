 ...
 * @subpackage Components
 */
class LocatePlacesModelTag extends JModel
{
	/**
	...
	function &getData()
	{
		// Load the data
		if (empty( $this->_data )) {
			$query = ' SELECT * FROM #__storelocator_tags '.
					'  WHERE id = '.$this->_id;
		...
		if (!$this->_data) {
			...
			$this->_data->marker_id = null;
			$this->_data->feature_marker_id = null;
		}
		return $this->_data;
	}
	...
	function delete()
	{
		...
		if (count( $cids )) {
			...
			$tagQuery = 'DELETE FROM `#__storelocator_tag_map` WHERE tag_id IN (' . implode(',',$cids) . ')';
