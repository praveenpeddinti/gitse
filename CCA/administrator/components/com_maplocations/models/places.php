...
class MapLocationsModelPlaces extends JModelList {
  ...
  protected function getListQuery() {
    ...
    $map_id = $app->input->get('map_id');
    if (!empty($map_id)) {
      $map_id = $db->Quote($db->escape($map_id, true));
      $query->where('(a.map_id = '.$map_id.')');
    }
    ...
    $orderCol = $app->input->get('ordering', 'a.title');
  ...
  protected function populateState($ordering = null, $direction = null) {
    ...
    $map_id = $this->getUserStateFromRequest($this->context.'.filter.map_id', 'filter_map_id', null, 'int');
    $this->setState('filter.map_id', $map_id);

    // Load the parameters.
    $params = JComponentHelper::getParams('com_maplocations');
