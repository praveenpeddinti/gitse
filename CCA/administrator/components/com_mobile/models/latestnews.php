...
class latestNewsUrlsModelLatestNews extends JModel{
	public $pagination;
	public $total;	
	public function getLatestNewsUrls(){
		try{
			...
			$db->setQuery("select * from ".$db->getPrefix()."latestnews limit ".$limitstart.",$limit");			
			//error_log("query is======================"."select * from #_latestnews order by id desc");
			$latestNewsUrls = $db->loadObjectList();
			$db->setQuery("select count(*) from ".$db->getPrefix()."latestnews");
			$total=$db->loadResult();
